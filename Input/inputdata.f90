...
module InputData
   ...
   !------------------------------------------------------------------------------------
   ! Multiscale + Demagnetization field related flags
   !------------------------------------------------------------------------------------
   ...
   character(len=1)   :: ms_demag             !< flag to enable multiscale demag (Y/N)
   integer            :: far_threshold        !< threshold for far-field calculation in ms_demag
   character(len=16)  :: ms_demag_solver      !< solver type for multiscale demag (direct/fft)
   character(len=16)  :: ms_demag_boundary    !< boundary condition for ms_demag (open/periodic)
   character(len=1)   :: ms_demag_output      !< flag to enable demag output (Y/N)
...
contains
   ...
   subroutine set_input_defaults()
      ...
      !Demagnetization field for multiscale
      far_threshold          = 20.0_dblprec
      ms_demag               = 'N'
      ms_demag_solver        = 'direct'
      ms_demag_boundary      = 'open'
      ms_demag_output        = 'N'
