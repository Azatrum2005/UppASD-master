   ...
   subroutine read_parameters(ifile)
      ...
      do
         ...
         if (comment) then
            ...
            case('far_threshold')
               read(ifile,*,iostat=i_err) far_threshold
               if(i_err/=0) write(*,*) 'ERROR: Reading ', trim(keyword),' data',i_err

            case('ms_demag')
               read(ifile,*,iostat=i_err) ms_demag
               if(i_err/=0) write(*,*) 'ERROR: Reading ', trim(keyword),' data',i_err

            case('ms_demag_solver')
               read(ifile,*,iostat=i_err) ms_demag_solver
               if(i_err/=0) write(*,*) 'ERROR: Reading ', trim(keyword),' data',i_err

            case('ms_demag_boundary')
               read(ifile,*,iostat=i_err) ms_demag_boundary
               if(i_err/=0) write(*,*) 'ERROR: Reading ', trim(keyword),' data',i_err

            case('ms_demag_output')
               read(ifile,*,iostat=i_err) ms_demag_output
               if(i_err/=0) write(*,*) 'ERROR: Reading ', trim(keyword),' data',i_err
            !------------------------------------------------------------------------
