...
contains
   ...
   subroutine run_measurement_phase()
      ...
      if(do_qb_chern=='Y') then
         call timing(0,'ChernNumber   ','ON')
         call calculate_qb_number(NA,Natom,Mensemble,simid,emomM,mmom,Nx,Ny,Nz,C1,C2,C3)
         call timing(0,'ChernNumber   ','OF')
      end if
	  
	  if (do_second_chern == 'Y') then
		 call timing(0,'SChernNumber  ','ON')
	     call calculate_second_chern_number(NA,Natom,Mensemble,simid,emomM,mmom,Nx,Ny,Nz,C1,C2,C3)
		 call timing(0,'SChernNumber  ','OF')
	  end if
   ...
   subroutine setup_simulation()
      ...
      !----------------------------!
    if (do_multiscale) then
     write(*,*) 'DEBUG: do_multiscale is TRUE, calling setup_multiscale_system'
     ...
     ! Initialize random number generator
      ...
      if (do_macro_cells.eq.'Y'.or.ham_inp%do_dip.eq.2) then
         ...
         ! ! In multiscale mode restrict macrocells to the atomistic+padding region (natom_atomistic)
         ! if (do_multiscale .and. natom_atomistic > 0) then
         !    call create_macrocell(NA,N1,N2,N3,natom_atomistic,Mensemble,block_size,coord, &
         !       Num_macro,max_num_atom_macro_cell,cell_index,macro_nlistsize,           &
         !       macro_atom_nlist,simid)
         ! else
         !    call create_macrocell(NA,N1,N2,N3,Natom,Mensemble,block_size,coord,        &
