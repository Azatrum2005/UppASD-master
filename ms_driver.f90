...
!******************************************************************
module ms_driver
   use MultiscaleDemag !DemagField 
   implicit none
   ...
   subroutine ms_iphase()
      ...
      do i=1,ipnphase
         ...
         do while(ipstep.LE.ipnstep(i))
            ...
            call build_ms_magnetization(emomM, 1)
            call calc_multiscale_demag()
            call add_multiscale_demag_to_beff(beff)
             ...
             call dampingBandPreinterpolation(dampingBand,emom)
              ...
              !    do_fixed_mom,'N',sot_btorque,dampingBand)
               ...
               Num_macro,cell_index,emomM_macro,macro_nlistsize,NA,N1,N2,N3)
                  ...
                  call build_ms_magnetization(emomM, 1)
                  call calc_multiscale_demag() !calculates the demag field for the finite difference mesh
                  call add_multiscale_demag_to_beff(beff) ! adds the demag field to the effective field
