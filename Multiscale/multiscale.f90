...
module Multiscale
  ...
  !Number of atomistic atoms (real + padding), excluding FD continuum nodes.
  integer :: natom_atomistic = 0
...
contains
  ...
  subroutine runMultiscaleSetup(options, setup,atomRegions)
    ...
    ! Save the number of atomistic + padding atoms for dipolar field calculation
    natom_atomistic = atomSetup%nrOfAtoms
    write(*,'(1x,a,i0)') 'Atomistic + padding atom count for dipolar field:    ', natom_atomistic
    ...
    !Initialize MS Demag
    write(*,*) 'DEBUG: runMultiscaleSetup reached setup_multiscale_demag call' 
...
!     call setup_multiscale_demag( mesh%nrOfGridPoints(1), mesh%nrOfGridPoints(2), mesh%nrOfGridPoints(3), &   !mesh%nrOfBoxes(1), mesh%nrOfBoxes(2), mesh%nrOfBoxes(3)
