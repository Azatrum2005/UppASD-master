...
contains
   ...
   subroutine dipole_field_calculation(NA,N1,N2,N3,Natom,do_dip,Num_macro,          &
      ...
      real(dblprec), dimension(:,:,:,:), intent(in) :: Qdip
      real(dblprec), dimension(:,:,:,:), intent(in):: Qdip_macro !< Matrix for the macro spin dipole-dipole interaction
      real(dblprec), intent(inout) :: energy
      real(dblprec), dimension(3,Natom,Mensemble), intent(inout) :: bfield

      integer :: ii,kk,icell
      integer :: actual_stop_atom, actual_natom
      ! In multiscale mode, clamp the atom range to the atomistic+padding region
      if (do_multiscale .and. natom_atomistic > 0) then
         actual_natom = natom_atomistic
         actual_stop_atom = min(stop_atom, natom_atomistic)
      else
         actual_natom = Natom
         actual_stop_atom = stop_atom
      end if
      ! write(*,'(a,7(i0,1x))') 'Dipolar field: ', Natom, natom_atomistic, actual_natom, start_atom, actual_stop_atom, Mensemble, size(Qdip,3)
      ...
      if (do_dip==1) then
         ...
         if (start_atom <= actual_stop_atom) then
            !$omp parallel do default(shared) private(ii,kk) reduction(-:energy) collapse(2) schedule(static)
            do kk=1, Mensemble
               do ii=start_atom, actual_stop_atom
                  ! Dipolar term
                  call dipolar_field(ii,kk,bfield(:,ii,kk),actual_natom,Mensemble,emomM,Qdip)
                  energy=energy-0.50_dblprec*(bfield(1,ii,kk)*emomM(1,ii,kk)+          &
                     bfield(2,ii,kk)*emomM(2,ii,kk)+bfield(3,ii,kk)*emomM(3,ii,kk))
               enddo
            enddo
   ...
   subroutine dipolar_field(i, k, field,Natom,Mensemble,emomM,Qdip)
      ...
      ! if (i == 1 .and. k == 1) then
      !    write(*,'(a,4(i0,1x))') 'Dipolar loop: ', i, k, Natom, size(Qdip,4)
      ! end if
