...
module HamiltonianActions
   ...
   use Parameters
   use HamiltonianData
   use InputData, only : ham_inp, do_multiscale
   use Multiscale, only : natom_atomistic       ! for dipolar field calculation
...
contains
   ...
   subroutine effective_field_full(Natom,Mensemble,start_atom,stop_atom,   &
      ...
      if (ham_inp%do_dip>0) then
         ...
         ! ! In multiscale mode restrict dipolar field loop to the atomistic+padding region only (natom_atomistic)
         ! if (do_multiscale .and. natom_atomistic > 0) then
         !    if (start_atom <= natom_atomistic) then
         !       call dipole_field_calculation(NA,N1,N2,N3,natom_atomistic,ham_inp%do_dip,Num_macro, &
         !          Mensemble,min(stop_atom,natom_atomistic),natom_atomistic,cell_index,macro_nlistsize,emomM, &
         !          emomM_macro,ham%Qdip,ham%Qdip_macro,energy,beff)
         !    end if
         ! else
         !    call dipole_field_calculation(NA,N1,N2,N3,Natom,ham_inp%do_dip,Num_macro,          &
   ...
   subroutine effective_field_single(Natom,Mensemble,i_atom, i_ensemble,  &
      ...
      if (ham_inp%do_dip>0) then
         ...
         ! ! In multiscale mode restrict to atomistic+padding region only.
         ! ! Skip this call entirely if i_atom is a continuum node.
         ! if (do_multiscale .and. natom_atomistic > 0) then
         !    if (i_atom <= natom_atomistic) then
         !       call dipole_field_calculation(NA,N1,N2,N3,natom_atomistic,ham_inp%do_dip,Num_macro, &
         !          Mensemble,i_atom,i_atom,cell_index,macro_nlistsize,emomM,        &
         !          emomM_macro,ham%Qdip,ham%Qdip_macro,energy,beff)
         !    end if
